...
class Board:
    ...
    def make_initial_grid(self):
        ...
        for i in range(self.gamesize):
            ...
            board_grid[self.gamesize-1][i] = RING
        return board_grid
    ...
    def check_and_generate_moves(self):
        ...
        for j in range(self.gamesize):
            ...
            if self.the_grid[self.gamesize-1][j] == CROSS:
                game_result = CROSS
    ...
    def __str__(self):
        ...
        for i in range(self.gamesize-1,-1,-1):
            # row number first
            return_str = return_str + str(i + 1) + '  '
            for j in range(self.gamesize-1):
    ...
    def cross_generate_forward_moves(self):
        ...
        '''
        moves = []
        for i in range(self.gamesize-1):
            for j in range(self.gamesize):
    ...
    def cross_generate_left_moves(self):
        ...
        '''
        moves = []
        for i in range(self.gamesize-2):
            for j in range(2,self.gamesize):
                if self.the_grid[i][j] == CROSS:
    ...
    def cross_generate_right_moves(self):
        ...
        moves = []
        for i in range(self.gamesize-2):
            for j in range(self.gamesize-2):
                if self.the_grid[i][j] == CROSS:
                    if self.the_grid[i + 2][j+2] == EMPTY and self.the_grid[i + 1][j + 1] != EMPTY:
                        new_board = Board(self)
                        new_board.the_grid[i][j] = EMPTY
    ...
    def ring_generate_left_moves(self):
        ...
        '''
        moves = []
        for i in range(2,self.gamesize):
            for j in range(2, self.gamesize):
                if self.the_grid[i][j] == RING:
                    if self.the_grid[i - 2][j - 2] == EMPTY and self.the_grid[i - 1][j - 1] != EMPTY:
                        new_board = Board(self)
