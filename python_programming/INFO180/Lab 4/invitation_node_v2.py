...
class InvitationNode:
    ...
    def is_consistent(self):
        ...
        # Constraint 1: ola => ~anne <=> ~ola or ~anne
        if self.relevant_constraint([self.anne, self.ola]):
            val = val and (self.ola.is_not_invited() or self.anne.is_not_invited())
        
        if self.relevant_constraint([self.kari, self.jan]):
            val = val and (self.kari.is_not_invited() or self.jan.is_invited())
            val = val and (self.jan.is_not_invited() or self.kari.is_invited())
        
        if self.relevant_constraint([self.ola, self.ivar, self.henning, self.knut]):
            val = val and (self.ola.is_not_invited() or self.ivar.is_not_invited() or self.henning.is_not_invited() or self.knut.is_not_invited())
            val = val and (self.ola.is_not_invited() or self.ivar.is_not_invited() or self.henning.is_not_invited() or self.knut.is_invited())
            val = val and (self.ola.is_not_invited() or self.ivar.is_not_invited() or self.henning.is_invited() or self.knut.is_not_invited())
            val = val and (self.ola.is_not_invited() or self.ivar.is_invited() or self.henning.is_not_invited() or self.knut.is_not_invited())
            val = val and (self.ola.is_invited() or self.ivar.is_not_invited() or self.henning.is_not_invited() or self.knut.is_not_invited())
            
        if self.relevant_constraint([self.ivar, self.randi, self.liv]):
            val = val and (self.ivar.is_not_invited() or self.liv.is_not_invited() or self.randi.is_not_invited())

        if self.relevant_constraint([self.gro, self.henning, self.kari]):
            val = val and (self.gro.is_not_invited() or self.henning.is_not_invited() or self.kari.is_not_invited())
            val = val and (self.gro.is_not_invited() or self.henning.is_not_invited() or self.kari.is_invited())
            val = val and (self.gro.is_not_invited() or self.henning.is_invited() or self.kari.is_not_invited())
        
        if self.relevant_constraint([self.gro, self.knut, self.jan]):
            val = val and (self.gro.is_not_invited or self.knut.is_not_invited() or self.jan.is_not_invited())

        if self.relevant_constraint([self.gro, self.anne, self.knut, self.ola]):
            val = val and (self.gro.is_not_invited or self.anne.is_not_invited() or self.knut.is_not_invited() or self.ola.is_not_invited())

        if self.relevant_constraint([self.ivar, self.henning, self.liv]):
            val = val and (not(self.ivar.is_not_invited or self.henning.is_invited() or self.liv.is_invited()) or not(self.ivar.is_invited() or self.henning.is_not_invited() or self.liv.is_not_invited())) 
